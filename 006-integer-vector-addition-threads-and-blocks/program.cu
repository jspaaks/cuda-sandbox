...
__global__ void add(int *a, int *b, int *c, int nElems) {
    ...
    if (iElem < nElems) {
        ...
        // we're indexing beyond the end of the array; do nothing
    }
}
...
int main(void) {
    ...
    if (nElems < nThreadsPerBlock) {
        // we need at least 1 block
        nBlocks = 1;
    } else {
        nBlocks = nElems / nThreadsPerBlock;
    }
    // call the integer add kernel with multiple blocks and threads, pass
    // it the values of d_a, d_b, as well as the (uninitialized) value 
    // of d_c
    add<<<nBlocks,nThreadsPerBlock>>>(d_a, d_b, d_c, nElems);
    ...
    // free up the memory on the host that we malloc'ed earlier.    
    free(h_a);
    free(h_b);
    free(h_c);
